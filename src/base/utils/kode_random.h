#ifndef kode_random_included
#define kode_random_included
//----------------------------------------------------------------------
...
/*
float   KODE_Random(void);
float   KODE_RandomSigned(void);
float   KODE_RandomRange(float minval, float maxval);
int32   KODE_RandomInt(void);
int32   KODE_RandomSignedInt(void);
...
/*
class KODE_Random_Rand {
  public:

    KODE_Random_Rand() {
    }

    virtual ~KODE_Random_Rand() {
    }
...
//static KODE_Random_Rand KODE_RANDOM;
//inline float KODE_Random(void) { return KODE_RANDOM.random(); }
...
float KODE_RandomSigned_System(void) {
  float r = (float)rand() * (float)KODE_INVRANDMAX;// / (float)RAND_MAX;
  return r * 2.0f - 1.0f;
}

// inclusive

float KODE_RandomRange_System(float minval, float maxval) {
  return minval + (KODE_Random() * (maxval - minval) );
}

//----------

int32_t KODE_RandomInt_System(void) {
  return rand();// & 0x7fffffff;
}

// -KODE_RandomMax..KODE_RandomMax

int32_t KODE_RandomSignedInt_System(void) {
  return rand();
}

// inclusive

int32_t KODE_RandomRangeInt_System(int32_t minval, int32_t maxval) {
  int32_t range = maxval-minval+1;
  if (range <= 0) return minval;
  //else return minval + SModulo(KODE_RandomInt(),range);
  else return minval + ( KODE_RandomInt() % range );
}
