#ifndef kode_dmc_included
#define kode_dmc_included
//----------------------------------------------------------------------
/*
...
#define KODE_DMC_THRESHOLD 2 // 8
#define KODE_DMC_BIGTHRESH 2 // 8
...
//struct kode_dmc_node {
//  float count[2];
//  kode_dmc_node* next[2];
//};
...
//------------------------------
  ...
  kode_dmc_node   *P;
  kode_dmc_node   *N;
  kode_dmc_node   nodes[256][256];
  kode_dmc_node   *nodebuf;
  kode_dmc_node   *nodemaxp;
  kode_dmc_node   *nodesptr;
...
//------------------------------
  ...
  KODE_Dmc() {
    ...
    wbufpos = 0;
    memsize   = 0x1000000;
    threshold = KODE_DMC_THRESHOLD;//2;
    bigthresh = KODE_DMC_BIGTHRESH;//2;
    nodebuf = KODE_NULL;
  }
...
//------------------------------
  ...
  void p_init(int memsize) {
    ...
    nodebuf = (kode_dmc_node *)KODE_Malloc(memsize);
    //if (nodebuf == (kode_dmc_node *)NULL)
    //{
    //  fprintf(stderr,"memory alloc failed; try smaller predictor memory\n");
    //  exit(1);
    //}
    nodemaxp = nodebuf + (memsize/sizeof(kode_dmc_node)) - 20;
    p_flush();
  }
