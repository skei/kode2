#ifndef kode_rgb_included
#define kode_rgb_included
//----------------------------------------------------------------------

#include "kode_math.h"
//typedef uint32_t KODE_BGRA;
...
uint32_t KODE_BGRA(uint8_t r, uint8_t g, uint8_t b, uint8_t a=255) {
  return
      ((uint32_t)a << 24)
    + ((uint32_t)r << 16)
    + ((uint32_t)g << 8 )
...
uint32_t KODE_BGRA_Add4(uint32_t c1, uint32_t c2, uint32_t c3, uint32_t c4) {
...
uint32_t KODE_BGRA_Average(uint32_t c1, uint32_t c2) {
...
uint32_t KODE_BGRA_Average2(uint32_t c1, uint32_t c2) {
...
uint32_t KODE_BGRA_ScaleAlpha(uint32_t c) {
  ...
  g = (g*a) >> 8;
  b = (b*a) >> 8;
  return KODE_BGRA(r,g,b,a);
}

//----------

uint32_t KODE_BGRA_Scale(uint32_t c, uint8_t a) {
  ...
  g = (g*a) >> 8;
  b = (b*a) >> 8;
  return KODE_BGRA(r,g,b,alpha);
}

//----------

uint32_t KODE_BGRA_Scale0(uint32_t c, uint8_t a) {
  //uint8_t alpha = (c & 0xff000000) >> 24;
  //a = 255 - a;
  uint32_t r = (c & 0xff0000) >> 16;
  uint32_t g = (c & 0x00ff00) >> 8;
  ...
  g = (g*a) >> 8;
  b = (b*a) >> 8;
  return KODE_BGRA(r,g,b,255);
}

//----------

uint32_t KODE_BGRA_Scale2(uint32_t c, uint8_t a) {
  uint8_t alpha = (c & 0xff000000) >> 24;
  a = (a*alpha) >> 8;
  //a = 255 - a;
  uint32_t r = (c & 0xff0000) >> 16;
...
/*
  used in (KODE_BGRA_Alpha):
    KODE_Bitmap.blendPixel
    KODE_Bitmap.premultAlpha
*/

//uint8_t KODE_BGRA_ScaleElement(uint32_t c, uint32_t a) {
//  uint32_t ret = (c*a) >> 8;
//  return ret & 0xff;
//}
...
/*
  used in  (KODE_BGRA_Scale):
    KODE_Bitmap.colorMatrix
*/

//uint8_t KODE_BGRA_ScaleF(uint8_t c, float n ) {
//  float nc = n * (float)c;
//  return KODE_MinI((uint8_t)255,(uint8_t)nc);
//}

//----------

//uint32_t KODE_BGRA_ScaleF2(uint32_t c, float a) {
//  uint8_t alpha = (c & 0xff000000) >> 24;
//  //a = 255 - a;
//  uint32_t r = (c & 0xff0000) >> 16;
//  uint32_t g = (c & 0x00ff00) >> 8;
...
uint32_t KODE_BGRA_AlphaBlend(uint32_t c1, uint32_t c2) {
...
uint32_t KODE_BGRA_AlphaBlend(uint32_t c1, uint32_t c2, uint8_t a) {
  ...
  g = (g*a) >> 8;
  b = (b*a) >> 8;
  c2 = KODE_BGRA(r,g,b); // was BGR
  return c1 + c2;
}

//----------

uint32_t KODE_BGRA_AlphaBlend2(uint32_t c1, uint32_t c2, uint8_t a) {
  ...
  g = (g*a) >> 8;
  b = (b*a) >> 8;
  c1 = KODE_BGRA(r,g,b,alpha); // was RGB
  a = 255-a;
  r = (c2 & 0xff0000) >> 16;
  g = (c2 & 0x00ff00) >> 8;
  b = (c2 & 0x0000ff);
  ...
  g = (g*a) >> 8;
  b = (b*a) >> 8;
  c2 = KODE_BGRA(r,g,b,alpha); // was BGR
  return c1+c2;
}

//----------

uint32_t KODE_BGRA_AlphaBlend3(uint32_t c1, uint32_t c2, uint8_t alpha) {
  alpha = 255-alpha;
  uint32_t a =  c1 & 0xff000000;
...
uint32_t KODE_BGRA_AlphaBlend4(uint32_t c1, uint32_t c2, uint8_t alpha) {
  uint8_t a = (c2 & 0xff000000) >> 24;
  alpha = (alpha*a) >> 8;
  //a = 255 - a;
...
uint32_t KODE_BGRA_AlphaBlend5(uint32_t c1, uint32_t c2, uint8_t alpha) {
  uint8_t a = (c2 & 0xff000000) >> 24;
  //alpha = (alpha*a) >> 8;
  //a = 255 - a;
  uint32_t r = (c2 & 0xff0000) >> 16;
...
uint32_t KODE_BGRA_AlphaBlend6(uint32_t c1, uint32_t c2, uint32_t a) {
  uint32_t r = (c1 & 0xFF0000) >> 16;
  uint32_t g = (c1 & 0x00FF00) >> 8;
  uint32_t b = c1 & 0x0000FF;
...
uint32_t KODE_BGRA_AlphaBlend7(uint32_t c1, uint32_t c2, uint32_t factor) {
...
uint32_t KODE_BGRA_BilinearAlphaBlend(uint32_t c1, uint32_t c2, uint32_t c3, uint32_t c4, int32_t bX, int32_t bY) {
...
uint32_t KODE_BGRA_BilinearAlphaBlend2(uint32_t c1, uint32_t c2, uint32_t c3, uint32_t c4, int32_t bX, int32_t bY) {
  int h = (bX & 0xff);
	int i = (bY & 0xff);
	bX = bX >> 8;
	bY = bY >> 8;
...
uint32_t KODE_BGRA_AddSaturate(uint32_t c1, uint32_t c2) {
...
uint32_t KODE_BGRA_AddSaturate2(uint32_t c1, uint32_t c2) {
  //uint32_t a =  c1 & 0xff000000;
...
uint32_t KODE_BGRA_AddSaturate3(uint32_t color1, uint32_t color2) {
...
uint32_t KODE_BGRA_SubSaturate(uint32_t c1, uint32_t c2) {
  uint32_t a =  c2 & 0xff000000;
...
uint32_t KODE_BGRA_MulSaturate(uint32_t c1, uint32_t c2) {
  uint32_t a =  c2 & 0xff000000;
  //c1 &= 0xffffff;
  //c2 &= 0xffffff;
...
uint32_t KODE_BGRA_Grey(uint32_t c) {
...
uint32_t KODE_BGRA_RgbToHsl(uint32_t color) {
  float r = (float)((color & 0xff0000) >> 16) * KODE_INV255F;
  float g = (float)((color & 0xff00  ) >> 8 ) * KODE_INV255F;
  float b = (float)((color & 0xff    )      ) * KODE_INV255F;
  ...
  s *= KODE_SQRT2;
  //printf("hsl: %.2f, %.2f, %.2f\n",h,s,l);
  return KODE_BGRA(h*256,s*256,v*256);
}
...
uint32_t KODE_BGRA_HslToRgb(uint32_t color) {
  float h = (float)((color & 0xff )) * KODE_INV255F;  // 0..1
  h *= KODE_PI2;                                               // 0..PI2
  h -= KODE_PI;                                                // -PI..PI
  float s = (float)((color & 0xff00 ) >> 8 ) * KODE_INV255F;  // 0..1
  s = s / KODE_SQRT2;                                          // 0..1/sqrt(2)
  float l = (float)((color & 0xff0000 ) >> 16 ) * KODE_INV255F;
  float u = cosf( h ) * s;
  float v = sinf( h ) * s;
  ...
  //printf("rgb: %.2f, %.2f, %.2f\n",r,g,b);
  return KODE_BGRA(r*256,g*256,b*256);
}

//----------

uint32_t KODE_BGRA_RgbToHsv(uint32_t color) {
  uint32_t a_ = (color & 0xff000000) >> 24;
  uint32_t r_ = (color & 0x00ff0000) >> 16;
  uint32_t g_ = (color & 0x0000ff00) >> 8;
  uint32_t b_ = (color & 0x000000ff);
  float r = (float)r_ * KODE_INV255F;
  float g = (float)g_ * KODE_INV255F;
  float b = (float)b_ * KODE_INV255F;
  //local c=r+g+b
  uint32_t c = r + g + b;
  //if c<1e-4 then return 0,2/3,0
  if (c < 0.001) { // 1e-4
    return KODE_BGRA(0,(2.0f/3.0f)*255,0,a_);
  }
  else {
    ...
    float s = p / (c+p);
    float v = (c+p) / 3.0f;
    return KODE_BGRA(h*255,s*255,v*255,a_);
  }
}

//----------

uint32_t KODE_BGRA_HsvToRgb(uint32_t color) {
  uint32_t a_ = (color & 0xff000000) >> 24;
  uint32_t h_ = (color & 0x00ff0000) >> 16;
  uint32_t s_ = (color & 0x0000ff00) >> 8;
  uint32_t v_ = (color & 0x000000ff);
  float h = (float)h_ * KODE_INV255F;
  h *= KODE_PI2;//360.0f;
  float s = (float)s_ * KODE_INV255F;
  float v = (float)v_ * KODE_INV255F;
  float r = v * (1 + s * (cosf(h) - 1));
  float g = v * (1 + s * (cosf(h - 2.09439) - 1));
  float b = v * (1 + s * (cosf(h + 2.09439) - 1));
  return KODE_BGRA(r*255,g*255,b*255,a_);
}

//----------

//function KODE_BGRA_HsvTween(h0,s0,v0,h1,s1,v1,t)
