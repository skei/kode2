#ifndef kode_neural_included
#define kode_neural_included
//----------------------------------------------------------------------
...
#define KODE_NEURAL_RANDOM_CLAMP (((float)rand()-(float)rand())/RAND_MAX)

// warning: integer overflow in expression [-Woverflow]

//returns a float between 0 & 1
#define KODE_NEURAL_RANDOM_NUM ((float)rand()/((uint32_t)RAND_MAX+1))
...
class KODE_Neural_Network {
  ...
  public:
    ...
    int SetData(float learning_rate,uint32_t layers[],uint32_t tot_layers) {
      ...
      Minputs = KODE_New float[layers[0]];
      Moutputs = KODE_New float[layers[tot_layers-1]];
    ...
    float* GetOutput(void) {
      float *outputs;
      uint32_t i,j,k;
      outputs = KODE_New float[Mlayers[Mtot_layers-1]];
    ...
    */
    ...
    float GetRand(void) {
      ...
      int seed = int(tblock->tm_sec+100*KODE_NEURAL_RANDOM_CLAMP+100*KODE_NEURAL_RANDOM_NUM);
      //srand(tblock->tm_sec);
      srand(seed);
      return (KODE_NEURAL_RANDOM_CLAMP+KODE_NEURAL_RANDOM_NUM);
    }
