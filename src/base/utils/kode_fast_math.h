#ifndef kode_fast_math_included
#define kode_fast_math_included
//----------------------------------------------------------------------
...
//#define KODE_MATH_EXTRA_PRECISION

float KODE_SqrtF(const float x); // KAsinF
...
float KODE_AcosF(const float x) {
  const float x2 = x*x;
...
float KODE_AsinF(const float x) {
  return KODE_PI05 - KODE_SqrtF(1 - x)*(1.5707288 - x*(0.2121144 + x*(0.0742610 -
  x*(0.0187293 + 0.395*x))));
}
...
float KODE_AtanF(const float x) {
  const float x2 = x*x;
...
//template<typename T>
float KODE_CosF4(float x) {
  float tp = KODE_INVPI2;
    x *= tp;
...
float KODE_SinF4(float x) {
  return KODE_CosF4(x-KODE_PI05);
}
...
float KODE_CoshF(const float x) {
...
float KODE_CoshF2(const float x) {
  const float x2 = x*x;
...
// is KODE_LITTLE_ENDIAN defined?

float KODE_ExpF(const float v) {
  union {
    double d;
    struct {
      // ???
      #ifdef KODE_LITTLE_ENDIAN
      int i,j;
      #else
      int j,i;
      #endif
...
float KODE_Inverse2_32(float x) {
...
float KODE_InvSqrtF(const float x) {
...
float KODE_InvSqrtF2_32(float x) {
...
float KODE_InvSqrtF3_32(float x) {
...
float KODE_Log2F(const float val) {
  if (val > 0.f) {
    union {
      int i;
      float j;
...
float KODE_Log10F(const float x) {
...
float KODE_LogF(const float &val) {
...
float KODE_PowF(float x, unsigned long n) {
  float res = 1;
  while (n > 0) {
    if (n & 1) res *= x;
    n >>= 1;
...
double KODE_PowF2(double a, double b) {
  union {
    double d;
    int x[2];
  } u = { a };
...
float KODE_SqrtF(const float x) {
...
float KODE_TanhF(const float x) {
  const float x2 = x*x;
