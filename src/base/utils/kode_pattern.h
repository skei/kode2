#ifndef kode_pattern_included
#define kode_pattern_included
//----------------------------------------------------------------------
...
// match defines
#define KODE_MATCH_PATTERN  6    // bad pattern
#define KODE_MATCH_LITERAL  5    // match failure on literal match
#define KODE_MATCH_RANGE    4    // match failure on [..] construct
#define KODE_MATCH_ABORT    3    // premature end of text string
#define KODE_MATCH_END      2    // premature end of pattern string
#define KODE_MATCH_VALID    1    // valid match

/* pattern defines */
#define KODE_PATTERN_VALID  0    /* valid pattern */
#define KODE_PATTERN_ESC   -1    /* literal escape at end of pattern */
#define KODE_PATTERN_RANGE -2    /* malformed range in [..] construct */
#define KODE_PATTERN_CLOSE -3    /* no end bracket in [..] construct */
#define KODE_PATTERN_EMPTY -4    /* [..] construct is empty */

//----------

const char* KODE_pattern_error_text[] =
{
  "[..] construct is empty",
...
class KODE_Pattern
{
  ...
  private:
    ...
    bool _is_valid_pattern(char* a_Pattern)
      {
        //int error_type = KODE_PATTERN_VALID;
        m_Error = KODE_PATTERN_VALID;
        char* p = a_Pattern;
        while (*p)
        {
          switch (*p)
          {
            ...
            case '[':
              ...
              if (*p==']')
              {
                m_Error = KODE_PATTERN_EMPTY;
                return false;
              }
              // if end of pattern here then bad pattern
              if (!*p)
              {
                m_Error = KODE_PATTERN_CLOSE;
                return false;
              }
    ...
    int _matcKODE_after_star(register char *p, register char *t)
      {
        register int mtch = 0;
        ...
        while ( *p == '?' || *p == '*' )
        {
          ...
          if (*p == '?')
          {
            ...
            if (!*t++) return KODE_MATCH_ABORT;
          }
          /* move to next char in pattern */
          p++;
        }
        /* if end of pattern we have matched regardless of text left */
        if (!*p) return KODE_MATCH_VALID;
        ...
        if (nextp == '\\')
        {
          ...
          if (!nextp) return KODE_MATCH_PATTERN;
        }
        ...
        do
        {
          ...
          if (!*t++) mtch = KODE_MATCH_ABORT;
        } while ( mtch != KODE_MATCH_VALID &&
                  mtch != KODE_MATCH_ABORT &&
                  mtch != KODE_MATCH_PATTERN);
        /* return result */
        return mtch;
      }
    ...
    int _match(register char *p, register char *t)
      {
        ...
        for ( ; *p; p++, t++)
        {
          ...
          if (!*t)
          {
            return ( *p=='*' && *++p =='\0' ) ? KODE_MATCH_VALID : KODE_MATCH_ABORT;
          }
          ...
          switch (*p)
          {
            ...
            case '*':
              return _matcKODE_after_star(p,t);
            ...
            case '[':
              {
                ...
                if (*p==']') { return KODE_MATCH_PATTERN; }
                member_match = false;
                loop = true;
                while (loop)
                {
                  ...
                  if (*++p=='-')
                  {
                    ...
                    if (range_end=='\0' || range_end==']') return KODE_MATCH_PATTERN;
                    // special character range end
                    if (range_end=='\\')
                    {
                      range_end = *++p;
                      // if end of text then we have a bad pattern
                      if (!range_end) return KODE_MATCH_PATTERN;
                    }
            ...
            default:
              if (*p != *t) return KODE_MATCH_LITERAL;
          } // switch (*p)
        }
        // if end of text not reached then the pattern fails
        if (*t) return KODE_MATCH_END;
        else  return KODE_MATCH_VALID;
      }
  ...
  public:
    ...
    bool match(char* a_Text)
      {
        if (m_ValidPattern)
        {
          ...
          return (m_Error==KODE_MATCH_VALID ) ? true : false;
        }
        else return false;
      }
    ...
    const char* getErrorText(void)
      {
        int index = m_Error+4;
        if (index>=0 && index<11) return KODE_pattern_error_text[index+4];
        return "";
      }
