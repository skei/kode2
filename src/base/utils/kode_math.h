#ifndef kode_math_included
#define kode_math_included
//----------------------------------------------------------------------
...
float KODE_Average(const unsigned int n, const float* ar) {
  float total = 0;
  unsigned int i=0;
  while (i<n) {
    total += ar[i];   i++;
...
float KODE_Clamp(const float input, const float limit) {
...
float KODE_Clamp(const float input, const float minval, const float maxval ) {
...
float KODE_Clip(float x, float l, float h) {
...
float KODE_Curve(float x, float t) {
  t = (t*0.990f) + 0.005f;
  float a = ( 1.0f - 1.0f/t );
  return x / (x+a * (x-1) );
}
...
float KODE_Fract(const float value) {
...
float KODE_Fract(const float value, float* intpart) {
...
float KODE_Max(const float a, const float b) {
  return (a > b) ? a : b;
}
...
float KODE_Min(const float a, const float b) {
  return (a < b) ? a : b;
}
...
float KODE_Modulo(float x, float y) {
...
//float KODE_Trunc(const float v); // see below

float KODE_Quantize(float AValue, int32_t ASteps) {
  ...
  //int32_t i = KODE_Trunc(v);
  int i = (float)(int)v;
  float res = (float)i / (float)(ASteps-1);
  return KODE_Min(res,1.0f);
}
...
float KODE_Rms(const unsigned int n, const float* ar) {
  float numr = 0;
  unsigned int i=0;
  while (i<n) {
    numr += ar[i]*ar[i];
...
float KODE_Round(const float v) {
...
/*
  KODE_Sigmoid(0.0)   //  0
  KODE_Sigmoid(0.1)   //  0.1495
  KODE_Sigmoid(0.3)   //  0.4365
  KODE_Sigmoid(0.5)   //  0.5875
  KODE_Sigmoid(0.7)   //  0.8785
  KODE_Sigmoid(0.9)   //  0.9855
  KODE_Sigmoid(1.0)   //  1
*/


float KODE_Sigmoid(float x) {
...
float KODE_Sign(const float v, const float def=0.0f) {
  //union {
  //  int32_t i;
  //  float f;
  //} u;
...
//double KODE_Sinc(double x) {
//  if (x==0) return 1;
//  else {
//    double pix = KODE_PI * x;
//    return sin(pix) / pix;
//  }
//}

float KODE_Sinc(float x) {
  if (x==0) return 1;
  else {
    float pix = KODE_PI * x;
    return sin(pix) / pix;
...
float KODE_Trunc(const float v) {
  return (float)(int)v;
}

//----------

float KODE_Wrap(float v, float lo, float hi) {
  if (lo > hi) {
    //KODE_Swap(lo,hi);
    float temp = lo;
    lo = hi;
...
//int32_t KODE_Remap(int32_t value, int32_t fromStart, int32_t fromEnd, int32_t toStart, int32_t toEnd) {
...
//  //return 440 * KPowF2(2.0,(ANote-69)*KODE_INV12F);  // too much error for notetohz
//}
