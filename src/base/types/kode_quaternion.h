...
void KODE_Quat_Normalize( float *q ) {
...
void KODE_Quat_Mult(float *qL, float *qR) {
  float qp[4];
...
void KODE_Quat_Inverse(float *q) {
  float l,norminv;
...
void KODE_Quat_ToMatrix(float *q, float *m) {
...
void KODE_Quat_FromAngAxis(float ang, KODE_Vector3 *axis, float *q) {
  float omega,s;
  omega = ang*0.5;
  s = sin(omega);
  q[qX] = s*axis->x;
...
void KODE_Quat_ToAngAxis(float *q, float *ang, KODE_Vector3 *axis) {
  KODE_Quat_Normalize(q);
...
float KODE_Quat_RelAngAxis(float *p, float *q, KODE_Vector3 *axis) {
  float ang;
  KODE_Quat_Inverse(p);
  KODE_Quat_Mult(p,q);
  KODE_Quat_ToAngAxis(p,&ang,axis);
  return(ang);
}
