#ifndef kode_linux_cpu_included
#define kode_linux_cpu_included
//----------------------------------------------------------------------

// http://bits.stephan-brumme.com/endianess.html
// see also: KODE_LITTLE_ENDIAN
...
class KODE_FpuState {
  ...
  public:
    ...
    KODE_FpuState(Rounding mode = kRoundToZero) {
...
// fPIC compatible
#define KODE_CPU_EBX_REG "r"

#define KODE_CPU_EBX_STORE  \
  "pushl %%ebx;"            \
  "cpuid;"                  \
  "movl %%ebx, %1;"         \
  "popl %%ebx;"
...
class SCPU {
  ...
  private:
    ...
    char          cpustringbuf[KODE_MAX_STRING_LENGTH];
    unsigned int  _caps;
