#ifndef kode_shm_included
#define kode_shm_included
//----------------------------------------------------------------------

#include <sys/shm.h>

class KODE_SharedMemory {
...
//------------------------------
  ...
  */
  ...
  KODE_SharedMemory(uint32_t ASize, const char* AFile) {
...
//------------------------------
  ...
  */
  ...
  void* attach(void) {
    void* ptr = shmat(MId,KODE_NULL,0);
    if ((intptr_t)ptr == -1) {
      perror("shmat");
      exit(1);
    }
