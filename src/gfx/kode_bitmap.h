...
//------------------------------
  ...
  KODE_Bitmap(uint32_t AWidth, uint32_t AHeight) {
    MWidth      = AWidth;
    MHeight     = AHeight;
    MStride     = MWidth * 4;
  ...
  KODE_Bitmap(KODE_Bitmap* ABitmap, uint32_t AX, uint32_t AY, uint32_t AW, uint32_t AH) {
    MWidth      = AW;
    MHeight     = AH;
    MStride     = MWidth * 4;
    MBufferSize = MStride * MHeight;
...
//------------------------------
  ...
  */
  ...
  KODE_Bitmap* getSubBitmap(KODE_Bitmap* ABitmap, int32_t x, int32_t y, int32_t w, int32_t h) {
    ABitmap->MWidth      = w;
    ABitmap->MHeight     = h;
...
//------------------------------
  ...
  void copyLine_NoClip(int32_t AXpos, int32_t AYpos, int32_t ALength, KODE_Bitmap* ABitmap, int32_t AXsrc, int32_t AYsrc) {
    uint32_t* dst = (uint32_t*)getPixelPtr(AXpos,AYpos);
  ...
  */
  ...
  void copyBitmap_NoClip(int32_t AXpos, int32_t AYpos, KODE_Bitmap* ABitmap) {
    for (uint32_t y=0; y<ABitmap->MHeight; y++) {
...
//------------------------------
  ...
  void blendLine(int32_t AXpos, int32_t AYpos, int32_t ALength, KODE_Bitmap* ABitmap, int32_t AXsrc, int32_t AYsrc) {
    ...
    if (_clipLine2(AXpos,ALength,AXsrc,&X,&L,&X2)) {
      ...
      for (int32_t x=0; x<L; x++) {
        ...
        uint32_t  c2 = *src++;
        uint8_t   a  = c2 >> 24;
        uint32_t  c  = KODE_BGRA_AlphaBlend(c1,c2,a);
        *dst++ = c;
      }
    }
  }
  ...
  void blendLine_NoClip(int32_t AXpos, int32_t AYpos, int32_t ALength, KODE_Bitmap* ABitmap, int32_t AXsrc, int32_t AYsrc) {
    uint32_t* dst = (uint32_t*)getPixelPtr(AXpos,AYpos);
    uint32_t* src = (uint32_t*)ABitmap->getPixelPtr(AXsrc,AYsrc);
    for (int32_t x=0; x<ALength; x++) {
      uint32_t  c1 = *dst;
  ...
  void blendLine(int32_t AXpos, int32_t AYpos, int32_t ALength, KODE_Bitmap* ABitmap, int32_t AXsrc, int32_t AYsrc, uint8_t AAlpha) {
    ...
    if (_clipLine2(AXpos,ALength,AXsrc,&X,&L,&X2)) {
      ...
      for (int32_t x=0; x<L; x++) {
        uint32_t  c1 = *dst;
        uint32_t  c2 = *src++;
        uint32_t  c  = KODE_BGRA_AlphaBlend(c1,c2,AAlpha);
        *dst++ = c;
      }
    }
  }
  ...
  void blendLine_NoClip(int32_t AXpos, int32_t AYpos, int32_t ALength, KODE_Bitmap* ABitmap, int32_t AXsrc, int32_t AYsrc, uint8_t AAlpha) {
    uint32_t* dst = (uint32_t*)getPixelPtr(AXpos,AYpos);
    uint32_t* src = (uint32_t*)ABitmap->getPixelPtr(AXsrc,AYsrc);
    for (int32_t x=0; x<ALength; x++) {
      uint32_t  c1 = *dst;
  ...
  */
  ...
  void blendBitmap_NoClip(int32_t AXpos, int32_t AYpos, KODE_Bitmap* ABitmap) {
    for (uint32_t y=0; y<ABitmap->MHeight; y++) {
  ...
  void blendBitmap_NoClip(int32_t AXpos, int32_t AYpos, KODE_Bitmap* ABitmap, uint8_t AAlpha) {
    for (uint32_t y=0; y<ABitmap->MHeight; y++) {
...
//------------------------------
  ...
  void rotozoom(KODE_Bitmap* ASrcBitmap, float AAngle, float AZoom, float ADstX=0, float ADstY=0, float ASrcX=0, float ASrcY=0, bool AWrap=false) {
...
//------------------------------
  ...
  #ifndef KODE_NO_BITMAP_DRAW
    #include "gfx/kode_bitmap_draw.h"
  #endif
