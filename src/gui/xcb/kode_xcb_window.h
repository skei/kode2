...
//------------------------------
  ...
  xcb_screen_t*               MScreen                       = KODE_NULL;
...
//------------------------------
  ...
  void createWindow(uint32_t AWidth, uint32_t AHeight, const char* ATitle, void* AParent=KODE_NULL) {
  ...
  */
  ...
  uint32_t remapButton(uint32_t AButton, uint32_t AState) {
    ...
    //KODE_Print("remapButton AButton %i AState %i\n",AButton,AState);
    uint32_t b = AButton;
    return b;
  }
  ...
  uint32_t remapState(uint32_t AState) {
    ...
    //if (AState & XCB_MOD_MASK_1) KODE_Print("1\n");
    //if (AState & XCB_MOD_MASK_2) KODE_Print("2\n");
    //if (AState & XCB_MOD_MASK_3) KODE_Print("3\n");
    //if (AState & XCB_MOD_MASK_4) KODE_Print("4\n");
    //if (AState & XCB_MOD_MASK_5) KODE_Print("5\n");
    return s;
  }
      ...
      case XCB_CLIENT_MESSAGE: {
        ...
        switch(data) {
          case KODE_THREAD_ID_TIMER:
            on_window_timer();
            break;
          case KODE_THREAD_ID_IDLE:
            if (MWindowMapped && MWindowExposed) {
              on_window_idle();
            }
            break;
          default:
            on_window_clientMessage(data,KODE_NULL);
            break;
        }
        break;
      }
  ...
  void* xcb_timer_thread_proc(void* AWindow) {
    ...
    if (window) {
      ...
      while (window->MTimerThreadActive) {
        ...
        window->sendEvent(KODE_THREAD_ID_TIMER,0);
