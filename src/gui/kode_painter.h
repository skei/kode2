...
public:
  ...
  void drawPie(KODE_FRect rect, float a1, float a2, uint32_t color) final {
    float cx = rect.x + (rect.w * 0.5f);
    float cy = rect.y + (rect.h * 0.5f);
    moveTo(cx,cy);
    //moveTo(cx,rect.y);
    arc(rect.x,rect.y,rect.w,rect.h,a1,a2,color);
    moveTo(cx,cy);
    //moveTo(cx,rect.y);
    setColor(color);
    stroke();
  ...
  void drawEllipse(KODE_FRect rect, uint32_t color) final {
    float cx = rect.x + (rect.w * 0.5f);
    //float cy = rect.y + (rect.h * 0.5f);
    //moveTo(cx,cy);
    moveTo(cx,rect.y);
    arc(rect.x,rect.y,rect.w,rect.h,0,1,color);
    //moveTo(cx,cy);
    //moveTo(cx,rect.y);
    setColor(color);
    stroke();
  ...
  void drawText(float x, float y, const char* text, uint32_t color) final {
    setColor(color);
    moveTo(x,y);
    fillText(text);
  }
  ...
  void drawText(KODE_FRect rect, const char* text, uint32_t color, uint32_t alignment) final {
    float x,y;
  ...
  void fillPie(KODE_FRect rect, float a1, float a2, uint32_t color) final {
    float cx = rect.x + (rect.w * 0.5f);
    float cy = rect.y + (rect.h * 0.5f);
    moveTo(cx,cy);
    arc(rect.x,rect.y,rect.w,rect.h,a1,a2,color);
