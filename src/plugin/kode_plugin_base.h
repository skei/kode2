...
public:
  virtual void      setDefaultParameterValues() { KODE_PRINT; }
...
public:
  ...
  virtual uint32_t  on_plugin_saveState(void** ABuffer, uint32_t AMode) { KODE_PRINT; *ABuffer = KODE_NULL; return 0; }
  virtual void      on_plugin_restoreState(uint32_t ASize, void* APointer, uint32_t AMode) { KODE_PRINT; }
};
...
public:
  ...
  virtual void updateParameterFromHost(uint32_t AIndex, float AValue) { KODE_PRINT; }
};
